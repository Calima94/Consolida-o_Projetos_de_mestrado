...
|---|---|---|---|
| [Capture_EMG_Data](https://github.com/Calima94/Capture_EMG_Data) | Coleta de sEMG (Myo) rotulada por ângulo de cotovelo via webcam + MediaPipe | Python 3, PyQt5, OpenCV, MediaPipe, pyserial | **Não portado nesta etapa.** O protocolo do Myo (comum aos três repositórios) foi portado em `mestrado_emg/myo_protocol.py` |
...
| 12 | Lei de controle não linear: `vel = kp·e + vel_atual·kd·e`; o ombro publicava só o erro (`kp` efetivo 1) | `arm_controller.py` | Controle P dentro do Gazebo (`JointPositionController` com comandos de velocidade), com os mesmos `kp`. O termo `kd` foi removido |
